...
use crate::{
    pow::{
        hasher::{Hasher, HeavyHasher, PowHasher},
        heavy_hash::Matrix,
    },
...
impl State {
    ...
    pub fn calculate_pow(&self) -> Uint256 {
        ...
        // 3) Extra heavy-hash round
        // 4) Domain-separated final squeeze + word mixing
        let stage1 = self.hasher.finalize_with_nonce(self.nonce);
        let stage2 = self.matrix.heavy_hash(stage1);
        let stage3 = HeavyHasher::hash(stage2);
        let stage4 = crate::pow::hasher::memory_hard_hash(stage3, stage1, self.nonce, self.block_target);
        ...
        final_state.update(&self.nonce.to_le_bytes());
    ...
    #[inline(always)]
    pub fn pow_match(&self) -> PowMatch {
    ...
    #[inline(always)]
    pub fn block_target(&self) -> Uint256 {
        self.block_target
    }

    #[inline(always)]
    pub fn share_target(&self) -> Uint256 {
        self.share_target
    }

    #[inline(always)]
    pub fn check_pow(&self) -> bool {
        self.pow_match() != PowMatch::None
    }

    #[inline(always)]
    pub fn generate_block_if_pow(&self) -> Option<RpcBlock> {
        matches!(self.pow_match(), PowMatch::Block).then(|| {
...
#[cfg(test)]
mod tests {
    use crate::pow::hasher::{Hasher, HeaderHasher, HeavyHasher};
    use crate::pow::serialize_header;
    use crate::proto::{RpcBlockHeader, RpcBlockLevelParents};
    use crate::{target::Uint256, Hash};

    fn u256_from_le_hex(hex: &str) -> Uint256 {
    ...
    fn u256_from_be_hex(hex: &str) -> Uint256 {
        let mut bytes = [0u8; 32];
        for i in 0..32 {
            bytes[31 - i] = u8::from_str_radix(&hex[(i * 2)..(i * 2 + 2)], 16).unwrap();
        }
        Uint256::from_le_bytes(bytes)
    }
    ...
    #[test]
    fn velkar_stratum_pow_regression() {
        let pre_pow_hash = u256_from_le_hex("f92b22c908fe912ef58501e2baa9253373d5d7f641232e8d4b386f61f4156a19");
        let timestamp = 1_781_329_471_115u64;
        let nonce = u64::from_str_radix("3a333445c075fa3d", 16).unwrap();
        ...
        let stage2 = state.matrix.heavy_hash(stage1);
        let stage3 = HeavyHasher::hash(stage2);

        let mut final_state = blake2b_simd::Params::new().hash_length(32).key(b"ProofOfWorkHash").to_state();
        final_state.update(&stage3.to_le_bytes());
        final_state.update(&state.nonce.to_le_bytes());
        final_state.update(&state.block_target.to_le_bytes());
        let stage4 = Uint256::from_le_bytes(final_state.finalize().as_bytes().try_into().unwrap());

        let pow = state.calculate_pow();

        println!("stage1={}", stage1.to_be_hex());
        println!("stage2={}", stage2.to_be_hex());
        println!("stage3={}", stage3.to_be_hex());
