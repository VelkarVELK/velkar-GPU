...
#[inline(always)]
pub fn memory_hard_hash(stage3: Hash, stage1: Hash, nonce: u64, target: crate::target::Uint256) -> Hash {
    ...
    password[..32].copy_from_slice(&stage3.to_le_bytes());
    password[32..64].copy_from_slice(&target.to_le_bytes());
    password[64..72].copy_from_slice(&nonce.to_le_bytes());

    let stage1_bytes = stage1.to_le_bytes();
    let stage3_bytes = stage3.to_le_bytes();
    let mut salt = [0u8; 16];
    for i in 0..16 {
