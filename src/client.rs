use crate::{
    miner::MinerManager,
    proto::{
        rpc_client::RpcClient, velkard_message::Payload, GetBlockTemplateRequestMessage, GetInfoRequestMessage,
        VelkardMessage,
    },
...
#[allow(dead_code)]
pub struct VelkardHandler {
    client: RpcClient<TonicChannel>,
    pub send_channel: Sender<VelkardMessage>,
    stream: Streaming<VelkardMessage>,
...
impl VelkardHandler {
    ...
    pub async fn client_send(&self, msg: impl Into<VelkardMessage>) -> Result<(), SendError<VelkardMessage>> {
        self.send_channel.send(msg.into()).await
    }

    pub async fn client_get_block_template(&mut self) -> Result<(), SendError<VelkardMessage>> {
    ...
    pub async fn listen(&mut self, miner: &mut MinerManager, shutdown: ShutdownHandler) -> Result<(), Error> {
        while let Some(msg) = self.stream.message().await? {
            ...
            match msg.payload {
                ...
                None => warn!("velkard message payload is empty"),
            }
        }
        Ok(())
    }
    ...
    async fn handle_message(&mut self, msg: Payload, miner: &mut MinerManager) -> Result<(), Error> {
        match msg {
            ...
            Payload::GetInfoResponse(info) => info!("Velkard version: {}", info.server_version),
