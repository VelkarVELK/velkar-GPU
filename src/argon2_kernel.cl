...
ulong b2b_rotr64(ulong x, uint n)
{
    return (x >> n) | (x << (64 - n));
}
...
ulong load64_le_private(__private const uchar *src)
{
    ulong v = 0;
    for (uint i = 0; i < 8; i++) {
        v |= ((ulong)src[i]) << (8 * i);
...
ulong load64_le_global(__global const uchar *src)
{
    ulong v = 0;
    for (uint i = 0; i < 8; i++) {
        v |= ((ulong)src[i]) << (8 * i);
...
void store32_le_private(__private uchar *dst, uint v)
{
    dst[0] = (uchar)(v);
    dst[1] = (uchar)(v >> 8);
    dst[2] = (uchar)(v >> 16);
...
void store64_le_private(__private uchar *dst, ulong v)
{
    for (uint i = 0; i < 8; i++) {
        dst[i] = (uchar)(v >> (8 * i));
    }
...
void store64_le_global(__global uchar *dst, ulong v)
{
    for (uint i = 0; i < 8; i++) {
        dst[i] = (uchar)(v >> (8 * i));
    }
...
void keccak_f1600_state(__private ulong *st)
{
    ulong bc[5];
    ulong t;
...
void store_u256_private(__private uchar *dst, __private const ulong *words)
{
    for (uint i = 0; i < 4; i++) {
...
ushort load_matrix_value(__global const uchar *constants, uint row, uint col)
{
    uint offset = 72 + (row * 64 + col) * 2;
    return (ushort)constants[offset] | ((ushort)constants[offset + 1] << 8);
}

void heavy_hash_words(__private ulong *out, __private const ulong *in_words)
{
    ulong state[25];
    for (uint i = 0; i < 25; i++) {
...
void matrix_heavy_hash_words(__private ulong *out, __global const uchar *constants, __private const ulong *input_words)
{
    uchar input[32];
    uchar nibbles[64];
    uchar product[32];
...
void blake2b_compress_private(__private ulong *h, __private const uchar *block, ulong t, bool last)
{
    ulong m[16];
    ulong v[16];
...
void blake2b_hash_private(__private uchar *out, uint out_len, __private const uchar *input, uint input_len)
{
    ulong h[8];
    uchar block[128];
...
void blake2b_hash32_prefix_block_global(__private uchar *out, __global const uchar *input)
{
    ulong h[8];
    uchar block[128];
...
void blake2b_keyed_pow_72(__private uchar *out, __private const uchar *input)
{
    ulong h[8];
    uchar block[128];
...
void digest_long_1024_global(__global uchar *out, __private const uchar *input72)
{
    uchar data[128];
    uchar buffer[64];
    uchar next[64];
                  ...
                  __local struct u64_shuffle_buf *buf)
{
    ...
    // Every lane must finish reading before the next shuffle overwrites the
